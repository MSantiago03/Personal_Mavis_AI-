...
import sys
import domains.hospital.state as hospital_state
...
class HospitalGoalDescription:
    ...
    def __init__(self, level, goals):
        ...
        # print("1Agent Goals:", file=sys.stderr)
        # print(self.agent_goals, file=sys.stderr)
        # print(self.box_goals, file=sys.stderr)
    ...
    def is_goal(self, state):
        ...
        for (goal_position, goal_char, is_positive_literal) in self.goals:
            # state = hospital_state.HospitalState(self.level, )
    ...
    def get_sub_goal(self, index):
        ...
        if index < num_box_goals:
            return print(HospitalGoalDescription(self.level, [self.box_goals[index]]))
        else:
            return print(HospitalGoalDescription(self.level, [self.agent_goals[index - num_box_goals]]))

    def num_sub_goals(self):
        """
    ...
    def create_new_goal_description_of_same_type(self, goals):
        """
