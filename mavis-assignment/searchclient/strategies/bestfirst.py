...
class FrontierBestFirst:
    ...
    def __init__(self):
        ...
        # Your code here...
        self.state = h_state.HospitalState
        self.set = set()
...
class FrontierAStar(FrontierBestFirst):
    ...
    def f(self, state: h_state.HospitalState, goal_description: h_goal_description.HospitalGoalDescription) -> int:
        # f(n) = h(n) + g(n)
        # h(n) is estimated cost of shortest path from n to a goal state (call h() func)
        # g(n) is path cost
        h_n = self.heuristic.h(state, goal_description)
        g_n = len(state.extract_plan())
        return h_n + g_n
